        ...
        layouter.assign_region(
            || "Poseidon_Permutation", |mut region| {
                ...
                | -> Result<(), Error> {
                    ...
                    let rc0 = F::from_str_vartime(ROUND_CONSTANTS[*constant_idx]).unwrap();
                    let rc1 = F::from_str_vartime(ROUND_CONSTANTS[*constant_idx + 1]).unwrap();
                    let rc2 = F::from_str_vartime(ROUND_CONSTANTS[*constant_idx + 2]).unwrap();
